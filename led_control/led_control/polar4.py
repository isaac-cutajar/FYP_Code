...
class LidarToLEDNode(Node):
    ...
    def interpolate_led_angles(self, led_sequence, anchors):
        """Linearly interpolate angles for all LEDs in the physical sequence."""
        full_map = {}
        ...
        for i in range(len(anchor_leds) - 1):
            ...
            if start_led not in led_sequence or end_led not in led_sequence:
                continue  # skip if not in physical layout
    ...
    def angle_to_led(self, angle_deg):
        closest_led = min(self.led_angle_map.keys(), key=lambda led: abs(self.led_angle_map[led] - angle_deg))
        return closest_led
