...
class LidarToLEDNode(Node):
    def __init__(self):
        ...
        # Existing sweep sequence for angular mapping
        self.led_sequence = list(range(1, 54)) + list(range(158, 86, -1))

        # >>> UPDATED: full range now includes 0..201 (i.e., adds 192–201)
        self.full_led_range = list(range(0, 202))
        # Convenience list for your added tail LEDs
        self.tail_leds = list(range(192, 202))
    ...
    def _color_from_distance(self, distance):
        """Map distance to (r,g,b) using existing logic."""
        # distance already accounts for 0.5m offset in caller
        if distance <= 0.1:
            return (255, 0, 0)
        elif distance >= 2.0:
            return (0, 255, 0)
    ...
    def pose_array_callback(self, msg):
        # --- ERROR OVERRIDE: all LEDs (including 192–201) RED ---
        ...
        # --- NO PEOPLE: all LEDs GREEN (fixes prior white) ---
        if len(msg.poses) == 0:
            command = "B," + ";".join(f"{i},0,255,0" for i in self.full_led_range) + "\n"
            self.ser.write(command.encode())
            print("[STATUS] No humans: All LEDs GREEN")
            print(f"[DEBUG] Sending {len(self.full_led_range)} LEDs")
            # print(command)  # uncomment if you want to see the full payload
            return

        # --- PEOPLE PRESENT: set sweep LEDs by angle, set 192–201 to closest person's color ---
        led_color_map = {}

        # Pick the closest person (Euclidean distance minus 0.5m offset, floored at 0)
        closest_dist = None
        closest_color = (0, 255, 0)  # default to green just in case

        for pose in msg.poses:
            x = pose.position.x
            y = pose.position.y
            distance = max(0.0, math.sqrt(x**2 + y**2) - 0.5)
            r, g, b = self._color_from_distance(distance)
            ...
            # Angular mapping for the main sweep
            ...
            for i_led in indices:
                led_color_map[i_led] = (r, g, b)

        # Apply closest person's color to the added tail LEDs 192–201
        for tled in self.tail_leds:
            led_color_map[tled] = closest_color

        if led_color_map:
            command = "B," + ";".join(
                f"{i},{r},{g},{b}" for i, (r, g, b) in sorted(led_color_map.items())
            ) + "\n"
            self.ser.write(command.encode())
            print(f"[POSEARRAY] Sent {len(led_color_map)} LEDs (including tail LEDs {self.tail_leds[0]}–{self.tail_leds[-1]})")
