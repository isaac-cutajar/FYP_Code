...
class BackgroundSubtractionNode(Node):
    def __init__(self):
        ...
        self.publisher_exclusion_zones = self.create_publisher(MarkerArray, 'exclusion_zone_markers', 10)
        self.publisher_point_counts = self.create_publisher(MarkerArray, 'point_count_markers', 10)
        ...
        self.max_cluster_points = 80  # Maximum points for a valid cluster
        ...
        # Define exclusion zones (areas to ignore)
        # Format: [(x_min, x_max, y_min, y_max), ...]
        self.exclusion_zones = [
            
            (-0.4, 0.08, 0.13, 0.8),   # Front working area
    ...
    def scan_callback(self, msg):
        # Publish exclusion zone visualization
        ...
        for i, zone in enumerate(self.exclusion_zones):
            ...
            # Calculate center and size of the zone
            marker.pose.position.x = (x_max + x_min) / 2
            marker.pose.position.y = (y_max + y_min) / 2
            marker.pose.position.z = 0.0  # Place at ground level
            ...
            zone_markers.markers.append(marker)
        
        self.publisher_exclusion_zones.publish(zone_markers)
        ...
        for i in range(len(ranges_np)):
            ...
            if xy_dist >= self.dynamic_threshold:
                # Calculate height based on angle and range
                height = range_curr * np.sin(angle)
                # Only include points above 0.5m (typical human height starts here)
                if height > 0.5:
                    # Check if point is in exclusion zone
                    if not self.is_in_exclusion_zone(x, y):
                        dynamic_points.append((x, y))

        clusters = self.cluster_dynamic_points(dynamic_points)
        
        # Create markers for all clusters with point counts
        cluster_markers = MarkerArray()
        for i, cluster in enumerate(clusters):
            # Calculate centroid for text placement
            centroid = np.mean(cluster, axis=0)
            
            # Create a text marker to show the point count
            text_marker = Marker()
            text_marker.header.frame_id = "laser"
            text_marker.header.stamp = self.get_clock().now().to_msg()
            text_marker.id = i + 1000  # Use high ID to avoid conflicts
            text_marker.type = Marker.TEXT_VIEW_FACING
            text_marker.action = Marker.ADD
            text_marker.pose.position.x = float(centroid[0])
            text_marker.pose.position.y = float(centroid[1])
            text_marker.pose.position.z = 1.5  # Place text above the person
            
            # Color based on whether the cluster meets the criteria
            if self.min_cluster_points <= len(cluster) <= self.max_cluster_points:
                # Valid cluster - green text
                text_marker.color.r = 0.0
                text_marker.color.g = 1.0
                text_marker.color.b = 0.0
            else:
                # Invalid cluster - red text
            ...
            text_marker.text = f"Points: {len(cluster)}"
            cluster_markers.markers.append(text_marker)
        
        # Publish cluster point count markers
        self.publisher_point_counts.publish(cluster_markers)
        
        # Filter clusters based on point count range
        valid_clusters = [c for c in clusters if self.min_cluster_points <= len(c) <= self.max_cluster_points]
        centroids = [np.mean(c, axis=0) for c in valid_clusters]

        # Update tracking with velocity
        new_tracked = []
        for person_pos in centroids:
            matched = False
        ...
        # Create person visualization markers
        ...
        for i, person in enumerate(self.tracked_people):
            ...
            body.color.g = 0.5
            body.color.b = 1.0
            person_markers.markers.append(body)
            ...
            head.color.g = 0.5
            head.color.b = 1.0
            person_markers.markers.append(head)
        ...
        # Publish person markers
        self.publisher_centroids.publish(person_markers)
    ...
    def cluster_dynamic_points(self, points):
        ...
        for i, pt in enumerate(points):
            ...
            for j in range(i + 1, len(points)):
                if used[j]:
                    continue
                if np.linalg.norm(np.array(pt) - np.array(points[j])) < self.cluster_distance_threshold:
