...
class HumanPresenceComparator(Node):
    def __init__(self):
        ...
        # RGBD horizontal field of view (Kinect v1 ~57 deg)
        ...
        # Subscriptions
        self.rgbd_sub = self.create_subscription(Point, '/rgbd_human_position_sensor', self.rgbd_callback, 10)
        self.lidar_sub = self.create_subscription(Point, '/lidar_human_position_sensor', self.lidar_callback, 10)
        ...
        self.get_logger().info('Human Presence Comparator Node initialized.')

    def rgbd_callback(self, msg: Point):
        self.rgbd_point = msg

    def lidar_callback(self, msg: Point):
        # Apply LiDAR → camera offsets
        transformed = Point()
    ...
    def check_consistency(self):
        rgbd_detected = self.rgbd_point is not None
        lidar_detected = self.lidar_point is not None and self.is_within_rgbd_fov(self.lidar_point)

        # Logic: inconsistent if LiDAR detects in FOV but RGBD doesn't, or vice versa
        ...
        # Reset points for next iteration
        self.rgbd_point = None
        self.lidar_point = None
