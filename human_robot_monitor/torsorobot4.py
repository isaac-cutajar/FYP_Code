...
class BackgroundSubtractionNode(Node):
    def __init__(self):
        ...
        self.cluster_distance_threshold = 0.3
        ...
        self.min_leg_sep = 0.2
        self.max_leg_sep = 0.6
    ...
    def scan_callback(self, msg):
        ...
        centroids = [np.mean(c, axis=0) for c in clusters if len(c) > 10]

        # Match clusters into pairs (legs) → people
        paired_centroids, unpaired_centroids = self.pair_clusters_as_people(centroids)
        people = list(paired_centroids)

        for leg in unpaired_centroids:
            matched = False
            for person in self.tracked_people:
                if np.linalg.norm(leg - np.array([person['x'], person['y']])) < 0.5:
                    matched = True
                    break
            if matched:
                people.append(leg)
    ...
    def pair_clusters_as_people(self, centroids):
        paired = []
        unpaired = []
        used = set()
        for i in range(len(centroids)):
            ...
            for j in range(i + 1, len(centroids)):
                ...
                if self.min_leg_sep < dist < self.max_leg_sep:
                    midpoint = (centroids[i] + centroids[j]) / 2
                    paired.append(midpoint)
                    used.update([i, j])
                    break
        for i in range(len(centroids)):
            if i not in used:
                unpaired.append(centroids[i])
