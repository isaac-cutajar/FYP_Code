...
class HumanDetectionNode(Node):
    ...
    def find_closest_robot_link(self, x_pos, y_pos):
        """
        Find the closest robot link to the given position.
        Returns the closest link name, its position, and the distance.
        """
        ...
        for link in self.robot_links:
            try:
                ...
                # Calculate distance to this link
                dist = np.linalg.norm([x_pos - lx, y_pos - ly])
                
                if dist < closest_dist:
                    closest_dist = dist
                    closest_link = link
                    closest_link_pos = (lx, ly)
            except Exception as e:
                self.get_logger().warn(f"Error looking up transform for {link}: {str(e)}")
                continue
                
        return closest_link, closest_link_pos, closest_dist
                    ...
                    if self.latest_depth_image is not None:
                        try:
                            ...
                            if len(valid_depths) > 0:
                                ...
                                # Find closest robot link
                                closest_link, closest_link_pos, min_distance = self.find_closest_robot_link(x_pos, y_pos)
                                
                                if closest_link is not None and closest_link_pos is not None:
                                    # Calculate relative position to closest link
                                    rel_x = x_pos - closest_link_pos[0]
                                    rel_y = y_pos - closest_link_pos[1]
                                    
                                    # For z-coordinate, we need to look up the z-position of the link
                                    try:
                                        tf = self.tf_buffer.lookup_transform("base_link", closest_link, rclpy.time.Time())
                                        link_z = tf.transform.translation.z
                                        rel_z = z_pos - link_z
                                    except Exception as e:
                                        self.get_logger().warn(f"Error looking up z-transform for {closest_link}: {str(e)}")
                                        rel_z = z_pos  # Use absolute z if transform lookup fails
