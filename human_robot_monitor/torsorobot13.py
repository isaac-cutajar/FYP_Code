...
from geometry_msgs.msg import TransformStamped, Point, Vector3, PoseArray, Pose
...
class BackgroundSubtractionNode(Node):
    def __init__(self):
        ...
        self.publisher_all_humans = self.create_publisher(PoseArray, 'all_human_positions', 10)
        ...
        self.max_cluster_points = 400  # Maximum points for a valid cluster
        ...
        # Add human detection timeout tracking
        self.last_detection_time = None
        self.detection_timeout = 2.0  # seconds
        self.detection_check_timer = self.create_timer(0.1, self.check_detection_timeout)  # Check every 100ms
    ...
    def check_detection_timeout(self):
        """Check if we haven't detected a human for longer than the timeout period"""
        if self.last_detection_time is None or (time() - self.last_detection_time) > self.detection_timeout:
            detection_msg = Int32()
            detection_msg.data = 0
            self.publisher_human_detected.publish(detection_msg)
            if self.last_detection_time is not None:
                self.get_logger().debug("No human detected for more than {} seconds".format(self.detection_timeout))
    ...
    def scan_callback(self, msg):
        ...
        if closest_person is not None and closest_link_pos is not None:
            ...
            self.publisher_human_detected.publish(detection_msg)
            self.last_detection_time = time()  # Update last detection time
            ...
            # Note: We don't publish human_detected = 0 here anymore, it's handled by the timer
        ...
        # Publish all human positions relative to base_link
        all_humans = PoseArray()
        all_humans.header.frame_id = "base_link"
        all_humans.header.stamp = self.get_clock().now().to_msg()
        
        for person in self.tracked_people:
            try:
                # Transform point from laser to base_link frame
                tf = self.tf_buffer.lookup_transform("base_link", "laser", rclpy.time.Time())
                
                # Create a pose for this person
                pose = Pose()
                
                # First transform from laser frame
                x_laser = person['x']
                y_laser = y = person['y']
                
                # Apply transform
                # Rotation
                theta = 2.0 * np.arctan2(tf.transform.rotation.z, tf.transform.rotation.w)  # Extract rotation angle
                x_base = x_laser * np.cos(theta) - y_laser * np.sin(theta)
                y_base = x_laser * np.sin(theta) + y_laser * np.cos(theta)
                
                # Translation
                x_base += tf.transform.translation.x
                y_base += tf.transform.translation.y
                
                pose.position.x = float(x_base)
                pose.position.y = float(y_base)
                pose.position.z = 0.0
                
                # Set orientation (we don't track orientation, so use default)
                pose.orientation.w = 1.0
                
                all_humans.poses.append(pose)
            except Exception as e:
                self.get_logger().warn(f"Could not transform human position: {str(e)}")
                continue
        
        # Publish all human positions
        self.publisher_all_humans.publish(all_humans)
